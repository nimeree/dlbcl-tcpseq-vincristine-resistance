...
outdir <- analysis_path("biotype_endRepair_pseudoreps_regenerated")
...
split_bam <- function(input_bam, label) {
  ...
  output_bams <- file.path(pseudorep_dir, paste0(prefix, "_ps", 1:2, ".bam"))

  if (!force_resplit && all(file.exists(output_bams), file.exists(paste0(output_bams, ".bai")))) {
    message("[2/5] Reusing existing regenerated pseudo-replicates for ", label)
    return(tibble(
      library_type = label,
      pseudorep = paste0("ps", 1:2),
      bam = output_bams,
      bam_name = basename(output_bams)
    ))
  }

  message("[2/5] Creating fresh pseudo-replicates for ", label)
  message("      reading qnames: ", input_bam)
  qnames <- scanBam(input_bam, param = ScanBamParam(what = "qname"))[[1]]$qname
  uq <- unique(qnames)
  ps1_qnames <- sample(uq, size = floor(length(uq) / 2), replace = FALSE)
  rm(qnames, uq)
  gc()

  ps1_qnames_current <<- ps1_qnames
  filters <- list(
    ps1 = expression(qname %in% get("ps1_qnames_current", envir = .GlobalEnv)),
    ps2 = expression(!(qname %in% get("ps1_qnames_current", envir = .GlobalEnv)))
  )
  ...
  rm(ps1_qnames)
  rm(ps1_qnames_current, envir = .GlobalEnv)
  gc()
...
write_csv(pseudorep_meta, file.path(outdir, "regenerated_pseudoreplicate_bams.csv"))
...
write_csv(biotype_counts, file.path(outdir, "RNA_biotype_endRepair_pseudorep_counts_long.csv"))
...
write_csv(summary_df, file.path(outdir, "RNA_biotype_endRepair_pseudorep_summary.csv"))
...
p <- ggplot(summary_df, aes(x = plot_biotype, y = mean_percent, fill = library_type)) +
  ...
  geom_errorbar(
    aes(ymin = pmax(mean_percent - se_percent, 0), ymax = mean_percent + se_percent),
...
ggsave(file.path(outdir, "RNA_biotype_endRepair_pseudoreps_regenerated.png"), p, width = 11, height = 8, dpi = 300)
ggsave(file.path(outdir, "RNA_biotype_endRepair_pseudoreps_regenerated.pdf"), p, width = 11, height = 8)
