...
for (frac in c("SSU", "RS", "DS")) {
  ...
  in_file <- file.path(frac_dir, paste0("Interaction_", frac, "_psite_limma_all_genes.csv"))
  if (!file.exists(in_file)) stop("Missing interaction table: ", in_file)
  ...
  fwrite(dt[significant_rawP0.05_lfc0.7 == TRUE],
         file.path(frac_dir, paste0("Interaction_", frac, "_psite_limma_sig_rawP0.05_lfc0.7.csv")))
  fwrite(dt[direction == "Up"],
         file.path(frac_dir, paste0("Interaction_", frac, "_psite_limma_sig_up_rawP0.05_lfc0.7.csv")))
  fwrite(dt[direction == "Down"],
         file.path(frac_dir, paste0("Interaction_", frac, "_psite_limma_sig_down_rawP0.05_lfc0.7.csv")))

  make_volcano(
    dt,
    title = paste0(frac, " P-site limma: true interaction"),
    out_png = file.path(frac_dir, paste0("Interaction_", frac, "_psite_limma_volcano.png"))
  )
  ...
  summary_rows[[frac]] <- data.table(
    ...
    plot = file.path(frac_dir, paste0("Interaction_", frac, "_psite_limma_volcano.png"))
  )
}

summary_dt <- rbindlist(summary_rows)
fwrite(summary_dt, file.path(out_dir, "psite_fraction_limma_true_interaction_plot_summary.csv"))

cat("\nGenerated true-interaction P-site fraction limma volcano plots:\n")
print(summary_dt)
