...
# Combined Up + Down significant genes for the true interaction.
# Produces cleaner, shorter output folders for:
#   1) P-site fraction limma counts: SSU, RS, DS
#   2) Translation metric limma: scanning, ribosome engagement, collision, protein output
# ============================================================
...
PSITE_OUT <- file.path(PSITE_DIR, "GO_BP_ORA_Interaction_Combined")
METRIC_OUT <- file.path(METRIC_DIR, "GO_BP_ORA_Interaction_Combined")
...
read_psite_query <- function(frac) {
  f <- file.path(PSITE_DIR, paste0("Fraction_", frac), paste0("Interaction_", frac, "_psite_limma_sig_rawP0.05_lfc0.7.csv"))
  if (!file.exists(f)) stop("Missing P-site significant table: ", f)
  dt <- fread(f)
...
read_metric_query <- function(metric) {
  f <- file.path(METRIC_DIR, "Results", metric, "Interaction_limma_all_genes.csv")
  if (!file.exists(f)) stop("Missing metric limma table: ", f)
  dt <- fread(f)
  if ("gene_id_clean" %in% names(dt)) setnames(dt, "gene_id_clean", "gene_id")
  if ("logFC" %in% names(dt)) setnames(dt, "logFC", "log2FoldChange")
  if (!"gene_name" %in% names(dt)) dt[, gene_name := NA_character_]
  dt[, gene_id := sub("\\.\\d+$", "", gene_id)]
  dt[, direction := fifelse(P.Value < P_CUT & log2FoldChange >= LFC_CUT, "Up",
                     fifelse(P.Value < P_CUT & log2FoldChange <= -LFC_CUT, "Down", "NS"))]
...
run_one_query <- function(dataset, key, query_dt, out_root) {
  ...
  message("[GO:BP ORA] ", dataset, " / ", nice, " (", length(query), " genes)")
  ...
  make_plot(filtered_dt, paste(label, "GO:BP ORA"), file.path(plots_dir, paste0(nice, "_GO_BP_ORA_combined_direction")))

  list(
    all = all_dt,
    filtered = filtered_dt,
    plotted = plotted_dt,
    query = query_dt[, .(dataset, analysis = key, analysis_label = label, gene_id, gene_name, log2FoldChange, direction)],
    summary = data.table(
      dataset = dataset,
      analysis = key,
      analysis_label = label,
...
psite_summary <- write_dataset_workbook(PSITE_OUT, psite_results, "GO_BP_ORA_Psite_fraction_limma_interaction_combined.xlsx")
...
metric_summary <- write_dataset_workbook(METRIC_OUT, metric_results, "GO_BP_ORA_translation_metric_limma_interaction_combined.xlsx")

cat("\nDone. GO:BP-only combined ORA outputs saved to:\n")
