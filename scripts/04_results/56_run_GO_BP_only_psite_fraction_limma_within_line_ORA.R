...
# Within-line contrasts only: Sensitive Vin vs DMSO and
# Resistant Vin vs DMSO.
# Up + Down significant genes are combined per fraction/contrast.
# ============================================================
...
OUT_DIR <- file.path(PSITE_DIR, "GO_BP_ORA_Within_Line_Combined")
...
run_one <- function(contrast, frac) {
  ...
  message("[Within-line GO:BP ORA] ", contrast_nice, " / ", frac, " (", length(query), " genes)")
  ...
  make_plot(
    ...
    file.path(plots_dir, paste0(frac, "_GO_BP_ORA_within_line_combined"))
  )
...
saveWorkbook(wb, file.path(OUT_DIR, "GO_BP_ORA_Psite_fraction_limma_within_line_combined.xlsx"), overwrite = TRUE)

cat("\nDone. Within-line GO:BP-only fraction-specific limma ORA outputs saved to:\n", OUT_DIR, "\n\n", sep = "")
cat("Summary:\n")
print(summary_dt)
