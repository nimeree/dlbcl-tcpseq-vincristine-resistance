...
out_dir <- file.path(base_dir, "Limma_translation_metrics_lfc0.7_rawP0.05", "Multi_metric_integration", "Full_cycle_IGV_style_LPXN_HNRNPD")
...
fwrite(top_tx, file.path(out_dir, "LPXN_HNRNPD_top_transcripts_for_coverage.csv"))
...
track_summary <- mean_cov[, .(
  ...
  total_mean_count = sum(mean_count),
  max_mean_cpm = max(mean_cpm),
  covered_positions = sum(mean_cpm > 0)
), by = .(gene_name, transcript, fraction, condition)]
fwrite(track_summary, file.path(out_dir, "LPXN_HNRNPD_igv_style_track_summary.csv"))
fwrite(mean_cov, file.path(out_dir, "LPXN_HNRNPD_mean_psite_coverage_by_position.csv"))
...
make_gene_plot <- function(gene) {
  ...
  p <- ggplot(d, aes(codon_pos, mean_cpm)) +
    ...
    labs(
      title = paste0(gene, " locus-level TCP-seq P-site coverage"),
