...
# Interaction: (Resistant Vin - Resistant DMSO) - (Sensitive Vin - Sensitive DMSO)
# ============================================================
...
OUT_DIR <- file.path(PSITE_LIMMA_DIR, "Pathway_gProfiler_Clean_REAC_GOBP_GOCC_true_interaction_lfc0.7_rawP0.05")
...
input_files <- unlist(lapply(c("SSU", "RS", "DS"), function(frac) {
  file.path(
    PSITE_LIMMA_DIR,
    paste0("Fraction_", frac),
    paste0("Interaction_", frac, "_psite_limma_sig_", c("up", "down"), "_rawP0.05_lfc0.7.csv")
  )
}))
input_files <- input_files[file.exists(input_files)]
if (length(input_files) == 0) stop("No true-interaction up/down CSV files found.")

parse_input_file <- function(f) {
  b <- basename(f)
  fraction <- sub("^Interaction_(SSU|RS|DS)_psite_limma_sig_.*$", "\\1", b)
  direction <- sub("^.*_sig_(up|down)_rawP0\\.05_lfc0\\.7\\.csv$", "\\1", b)
  data.table(
    file = f,
    comparison = "Interaction",
    comparison_label = "True interaction: resistant VCR response vs sensitive VCR response",
    fraction = fraction,
    fraction_label = FRACTION_LABELS[[fraction]],
    direction = stringr::str_to_title(direction)
  )
}

inputs <- rbindlist(lapply(input_files, parse_input_file), fill = TRUE)
setorder(inputs, fraction, direction)
...
for (i in seq_len(nrow(inputs))) {
  ...
  message("[g:Profiler P-site fraction interaction] ", key, " (", length(query), " genes)")

  plot_subdir <- file.path(PLOT_DIR, info$fraction, info$direction)
...
saveWorkbook(wb, file.path(OUT_DIR, "gProfiler_clean_REAC_GOBP_GOCC_true_interaction_lfc0.7_rawP0.05.xlsx"), overwrite = TRUE)

cat("\nDone. Clean pathway outputs saved to:\n", OUT_DIR, "\n\n", sep = "")
cat("Summary:\n")
print(summary_dt)
