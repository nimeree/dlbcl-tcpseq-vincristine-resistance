...
# - Small-n reporting threshold: FDR < 0.1
# ============================================================
...
OUT_DIR <- file.path(BASE_DIR, "Limma_translation_metrics_lfc0.7_fdr0.1")
...
make_volcano <- function(res, metric, contrast) {
  ...
  plot_dt[, call := fifelse(adj.P.Val < FDR_CUT & logFC >= LFC_CUT, "Up",
    fifelse(adj.P.Val < FDR_CUT & logFC <= -LFC_CUT, "Down", "NS"))]
  ...
  p <- ggplot(plot_dt, aes(x = logFC, y = neglog10_p)) +
    ...
    labs(
      ...
      subtitle = paste0("limma eBayes(trend=TRUE); significant: FDR < ", FDR_CUT, " and |logFC| >= ", LFC_CUT),
      x = "limma logFC",
      y = "-log10(P value)",
      color = NULL
    ) +
  ...
  out_base <- file.path(PLOT_DIR, metric, paste0(contrast, "_volcano_limma_lfc0.7_fdr0.1"))
...
run_metric_limma <- function(dt, metric) {
  ...
  for (contrast in colnames(contrasts_mat)) {
    ...
    res[, significant_fdr0.1_lfc0.7 := adj.P.Val < FDR_CUT & abs(logFC) >= LFC_CUT]
    res[, direction := fifelse(significant_fdr0.1_lfc0.7 & logFC >= LFC_CUT, "Up",
      fifelse(significant_fdr0.1_lfc0.7 & logFC <= -LFC_CUT, "Down", "NS"))]
    setcolorder(res, c(
      "gene_id_clean", "gene_name", "logFC", "AveExpr", "t", "P.Value",
      "adj.P.Val", "B", "rank_score", "significant_fdr0.1_lfc0.7", "direction"
    ))
    fwrite(res, file.path(metric_result_dir, paste0(contrast, "_limma_all_genes.csv")))
    fwrite(res[significant_fdr0.1_lfc0.7 == TRUE], file.path(metric_result_dir, paste0(contrast, "_limma_sig_fdr0.1_lfc0.7.csv")))
  ...
  writeData(wb, "Methods", data.table(
    item = c("model", "variance_moderation", "fdr_cutoff", "logfc_cutoff", "genes_in_matrix"),
    value = c("limma model.matrix(~ 0 + group)", "eBayes(trend=TRUE)", FDR_CUT, LFC_CUT, nrow(mat))
  ))
  saveWorkbook(wb, file.path(metric_result_dir, paste0(metric, "_limma_results_lfc0.7_fdr0.1.xlsx")), overwrite = TRUE)
...
summary_dt <- rbindlist(lapply(analysis, function(x) {
  rbindlist(lapply(names(x$results), function(contrast) {
    ...
    data.table(
      ...
      significant_fdr0.1_lfc0.7 = res[significant_fdr0.1_lfc0.7 == TRUE, .N],
...
fwrite(summary_dt, file.path(OUT_DIR, "limma_metric_contrast_summary.csv"))

interaction_hits <- rbindlist(lapply(METRICS, function(metric) {
  res <- analysis[[metric]]$results[["Interaction"]]
  res[significant_fdr0.1_lfc0.7 == TRUE, .(
    gene_id_clean,
    gene_name,
    metric,
    logFC,
...
if (nrow(interaction_hits)) {
  ...
  membership <- Reduce(function(x, y) merge(x, y, by = "gene_id_clean", all.x = TRUE), list(membership, wide_logfc, wide_fdr))
...
method_note <- c(
  ...
  "Significant genes are reported at FDR < 0.1 and |logFC| >= 0.7.",
...
message("Done. limma outputs written to: ", OUT_DIR)
