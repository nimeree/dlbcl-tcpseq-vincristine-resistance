...
# ============================================================
# g:Profiler ORA for limma translation metric interaction lists
# Up and Down genes combined into one query per metric.
...
RESULTS_DIR <- file.path(LIMMA_DIR, "Results")
OUT_DIR <- file.path(LIMMA_DIR, "Pathway_gProfiler_Clean_REAC_GOBP_GOCC_interaction_combined_direction_lfc0.7_rawP0.05")
...
read_query <- function(metric) {
  f <- file.path(RESULTS_DIR, metric, paste0(CONTRAST, "_limma_all_genes.csv"))
  if (!file.exists(f)) stop("Missing limma metric file: ", f)
  dt <- fread(f)
  ...
  dt[, direction := fifelse(P.Value < 0.05 & log2FoldChange >= 0.7, "Up",
                     fifelse(P.Value < 0.05 & log2FoldChange <= -0.7, "Down", "NS"))]
...
for (metric in names(METRIC_LABELS)) {
  query_dt <- read_query(metric)
  query <- unique(query_dt$gene_id)
  key <- paste(CONTRAST, metric, "CombinedDirection", sep = "_")
  message("[g:Profiler metric combined direction] ", key, " (", length(query), " genes)")

  plot_subdir <- file.path(PLOT_DIR, metric)
  ...
  if (length(query) == 0) {
    ...
    if (is.null(gost_result) || is.null(gost_result$result) || nrow(gost_result$result) == 0) {
      ...
      gost_dt[, `:=`(
        comparison = CONTRAST,
        comparison_label = "Differential VCR response in resistant vs sensitive",
        metric = metric,
  ...
  title <- paste(METRIC_LABELS[[metric]], "interaction combined-direction pathway ORA")
...
preferred_cols <- c(
  "comparison", "comparison_label", "metric", "metric_label", "direction",
...
saveWorkbook(wb, file.path(OUT_DIR, "gProfiler_REAC_GOBP_GOCC_limma_translation_metrics_interaction_combined_direction_lfc0.7_rawP0.05.xlsx"), overwrite = TRUE)

cat("\nDone. Combined-direction translation metric pathway outputs saved to:\n", OUT_DIR, "\n\n", sep = "")
cat("Summary:\n")
print(summary_dt)
